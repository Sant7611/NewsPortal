...
export const createNews = asyncErrorHandler(async (req, res, next) => {
  ...
  const categoryID = "6890a5057fd2104222b95124";
...
// ===================== Fetch Approved News (Public) - THIS IS YOUR HOMEPAGE =====================
export const fetchAllNewsPublicly = asyncErrorHandler(
  async (req, res, next) => {
    const news = await News.find({ status: "approved" })
      .sort({ createdAt: -1 }) // ✅ Latest first - MOST IMPORTANT
      .populate("category", "categoryName")
      .populate("journalist", "name email");
...
export const fetchAllNewsByCategory = asyncErrorHandler(
  async (req, res, next) => {
...
export const fetchAllNewsCreatedByJournalist = asyncErrorHandler(
  async (req, res, next) => {
    const journalistID = req.user._id;
    if (!journalistID) {
...
export const updateNewsCreatedByJournalist = asyncErrorHandler(
  async (req, res, next) => {
    const journalistID = req.user._id;
    const { newsID } = req.params;
    ...
    if (!news) {
      throw new customError(
        "Unauthorized, you are not allowed to edit this news",
        401
      );
    }

    const update = await News.findByIdAndUpdate(newsID, updatedData, {
      new: true,
      runValidators: true,
    })
      .populate("category", "categoryName")
...
export const deleteNewsCreatedByJournalist = asyncErrorHandler(
  async (req, res, next) => {
    const journalistID = req.user._id;
    const { newsID } = req.params;
    ...
    if (!news) {
      throw new customError(
        "You are not authorized to delete this news article",
        401
      );
    }
...
export const updateNewsStatus = async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      msg: `News status updated to ${status}`,
      news: updated,
    });
  } catch (error) {
    console.error(error);
...
export const mostPopularArticle = asyncErrorHandler(async (req, res, next) => {
  // Sort by viewCount descending, then by creation date
  const popularArticle = await News.find({ status: "approved" })
    .sort({ viewCount: -1, createdAt: -1 }) // ✅ Most viewed first, then latest
    .limit(5) // Remove skip(4) - just get top 5
    .populate("journalist", "name email")
    .populate("category");
